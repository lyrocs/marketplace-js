...
<template>
  ...
  >
    ...
    <div class="flex grow flex-col p-4">
      <h3 class="my-2 h-14 text-lg font-bold text-gray-800 hover:text-slate-600 overflow-hidden">
        {{ product.name }}
      </h3>
      ...
      <div class="flex-1 flex gap-4 border-t mt-2">
        <div
          ...
          :key="shop.id"
          class="mt-2 flex flex-col items-center gap-2 py-2"
        >
          <img
          ...
          <p class="text-2xl font-bold text-gray-900 flex-1 text-right">
            {{ shop.price }} {{ shop.currency === 'EUR' ? '€' : '$' }}
          </p>
        </div>
        <div class="flex-1 flex flex-col items-end justify-end gap-2">
