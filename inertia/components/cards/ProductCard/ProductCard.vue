<script setup lang="ts">
interface Props {
  product: any
  selectable?: boolean
  selected?: boolean
}

const props = withDefaults(defineProps<Props>(), {
  selectable: false,
  selected: false,
})

const emit = defineEmits<{
  select: [product: any]
  seeDetails: [product: any]
}>()
</script>

<template>
  <a :href="selectable ? '#' : `/product/${product.id}`" class="product-card group">
    <img
      :src="
        product.images ? product.images[0] : 'https://placehold.co/400x300/475569/white?text=Image'
      "
      class="product-card-image"
      alt="Product image"
    />
    <div class="product-card-content">
      <h3 class="product-card-title">{{ product.name }}</h3>
      <div class="product-card-specs">
        <!-- <p>{{ product.brand.name }}</p> -->
        <template v-if="product.specs.length > 0">
          <p class="product-card-specs-label">{{ $t('product.specs') }}:</p>
          <ul v-for="spec in product.specs" :key="spec.id" class="tag product-card-spec-tag">
            <li>{{ spec.value }}</li>
          </ul>
        </template>
      </div>
      <div class="product-card-shops">
        <div v-for="shop in product.shops" :key="shop.id" class="product-card-shop">
          <img
            src="data:image/png;base64,"
            class="product-card-shop-avatar"
            alt="product shop logo"
          />
          <span class="tag">
            {{ shop.available ? $t('product.available') : $t('product.unavailable') }}
          </span>
          <p class="product-card-shop-price">
            {{ shop.price }} {{ shop.currency === 'EUR' ? '€' : '$' }}
          </p>
        </div>
        <div class="product-card-actions">
          <Button @click="emit('seeDetails', product)">{{ $t('product.seeDetails') }}</Button>
          <Button
            v-if="selectable"
            @click="emit('select', product)"
            :variant="selected ? 'ghost' : 'secondary'"
            :disabled="selected"
          >
            {{ selected ? $t('product.selected') : $t('product.select') }}
          </Button>
        </div>
      </div>
    </div>
  </a>
</template>

<style scoped>
.product-card {
  @apply flex flex-col overflow-hidden rounded-xl bg-white shadow-lg transition-all duration-200;
}
.product-card-image {
  @apply h-48 w-full object-cover transition-transform duration-300 group-hover:scale-105;
}

.product-card-content {
  @apply flex grow flex-col p-4;
}

.product-card-title {
  @apply my-2 h-14 text-lg font-bold text-gray-800 hover:text-slate-600 overflow-hidden;
}

.product-card-specs {
  @apply flex gap-2 h-5;
}

.product-card-specs-label {
  @apply text-sm flex gap-1;
}

.product-card-spec-tag {
  @apply flex gap-1 h-5;
}

.product-card-shops {
  @apply flex-1 flex gap-4 border-t mt-2;
}

.product-card-shop {
  @apply mt-2 flex flex-col items-center gap-2 py-2;
}

.product-card-shop-avatar {
  @apply size-8 rounded-full;
}

.product-card-shop-price {
  @apply text-2xl font-bold text-gray-900 flex-1 text-right;
}

.product-card-actions {
  @apply flex-1 flex flex-col items-end justify-end gap-2;
}
</style>
